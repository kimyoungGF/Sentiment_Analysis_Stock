...
tokenizer1 = AutoTokenizer.from_pretrained('snunlp/KR-FinBert-SC')
model1=AutoModelForSequenceClassification.from_pretrained('snunlp/KR-FinBert-SC')
senti_classifier = pipeline('text-classification',model=model1,tokenizer=tokenizer1,device=0)
...
PATH = 'D:\stock\model/'
model2 = torch.load(PATH + 'model2.pt') 
...
# 이 함수는 불용어를 제거한 후 내용이 5자 이상인 행을 반환합니다.


def remove_stopwords_and_filter(df, stopwords, min_length=5):
    cleaned_texts = []
    indices_to_keep = []
    
    for index, text in enumerate(df['내용']):
        hangul = re.compile('[^가-힣]+')
        korean_text = hangul.sub(' ', str(text))  # 공백으로 대체
        cleaned_text = ' '.join([word for word in korean_text.split() if word not in stopwords])
        if len(cleaned_text) >= min_length and cleaned_text.strip() != '':
            cleaned_texts.append(cleaned_text)
            indices_to_keep.append(index)
    
    # 빈 문자열로 대체된 텍스트를 삭제하고 데이터프레임을 업데이트
    df = df.iloc[indices_to_keep]  # 인덱스를 사용하여 데이터프레임을 필터링
...
def storkbart(company):
    ...
    while True:
        alldf = pd.DataFrame()
        try:
            for csvnum in range(0, 500):
                try:
                    nowdf = pd.read_csv(f'D:\\stock\\주토방\\{company}{allcsvnum * 100}.csv', encoding='euc-kr',
                                        names=['날짜', '내용'])
                    alldf = pd.concat([alldf, nowdf], ignore_index=True)
                    allcsvnum += 1
                except UnicodeDecodeError as e:
                    # UnicodeDecodeError가 발생하면 해당 파일을 건너뜁니다.
                    print(f"파일 {company}{allcsvnum * 100}.csv을(를) 읽는 도중 오류가 발생했습니다: {str(e)}")
                    allcsvnum += 1  # 오류가 발생하더라도 다음 파일로 넘어갑니다.
        ...
        alldf = alldf[alldf['날짜'] <= 20230631]
        alldf = alldf[alldf['날짜'] >= 20210630]
        ...
        alldf = alldf.loc[alldf['글자수'] > 5]
        alldf = alldf.loc[alldf['글자수'] < 256]
        
        alldf = remove_stopwords_and_filter(alldf, stopwords, min_length=5)
        
        alldf['차별 분류'] = alldf['내용'].apply(lambda sentence: sentence_predict(sentence).item())
        

        # Define a list of values to filter for removal
        values_to_remove = [0, 2, 7]

        alldf = alldf[~alldf['차별 분류'].isin(values_to_remove)]
        ...
        for day in df_day_listint:
            ...
            inre=[day,neutralP,negativeP,positiveP]
            
            with open(f'D:\stock/감성분석/{company}.csv', 'a', newline='') as f_object:  
                writer_object = writer(f_object)
                writer_object.writerow(inre)  
                f_object.close()
...
a=[0,1,3,4,11,12,14,18,23,30,34,35,36,40,41,44,45,47,48]
b=[0]
c=[1,3,4,11,12]
d=[14,18,23,30,34,35]
f=[36,40,41,44,45,47,48]
